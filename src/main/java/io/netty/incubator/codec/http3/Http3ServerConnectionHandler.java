 ...
 * <a href="https://tools.ietf.org/html/draft-ietf-quic-http-32">HTTP3</a> for
 * the server-side.
 */
public class Http3ServerConnectionHandler extends Http3ConnectionHandler {
  private final ChannelHandler requestStreamHandler;

  /**
   * Create a new instance.
   *
   * @param requestStreamHandler the {@link ChannelHandler} that is used for each
   *                             new request stream.
   *                             This handler will receive
   *                             {@link Http3HeadersFrame} and
   ...
   * Create a new instance.
   * 
   * @param requestStreamHandler               the {@link ChannelHandler} that is
   *                                           used for each new request stream.
   *                                           This handler will receive
   *                                           {@link Http3HeadersFrame} and
   *                                           {@link Http3DataFrame}s.
   * @param inboundControlStreamHandler        the {@link ChannelHandler} which
   *                                           will be notified about
   *                                           {@link Http3RequestStreamFrame}s or
   *                                           {@code null} if the user is not
   *                                           interested in these.
   * @param unknownInboundStreamHandlerFactory the {@link LongFunction} that will
   *                                           provide a custom
   *                                           {@link ChannelHandler} for unknown
   *                                           inbound stream types or
   *                                           {@code null} if no special handling
   *                                           should be done.
   * @param localSettings                      the local
   *                                           {@link Http3SettingsFrame} that
   *                                           should be sent to the
   *                                           remote peer or {@code null} if the
   *                                           default settings should be used.
   * @param disableQpackDynamicTable           If QPACK dynamic table should be
   *                                           disabled.
   */
  public Http3ServerConnectionHandler(ChannelHandler requestStreamHandler,
