...
import org.almond.webtransport.DefaultWebtransportBidStreamFrame;
import org.almond.webtransport.DefaultWebtransportUnidStreamFrame;
...
import static org.almond.webtransport.CodecUtils.WEBTRANSPORT_FRAME_TYPE_ID;
import static org.almond.webtransport.CodecUtils.WEBTRANSPORT_STREAM_TYPE;
...
final class Http3FrameCodec extends ByteToMessageDecoder implements ChannelOutboundHandler {
  ...
  private static final InternalLogger logger = InternalLoggerFactory
      .getInstance(Http3FrameCodec.class);
  ...
  private boolean WEBTRANSPORT_BID_STREAM = false;
  private boolean WEBTRANSPORT_UNID_STREAM = false;
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
    ...
    if (WEBTRANSPORT_BID_STREAM) {
      out.add(new DefaultWebtransportBidStreamFrame(in.readRetainedSlice(in.readableBytes())));
      return;
    }
    if (WEBTRANSPORT_UNID_STREAM) {
      out.add(new DefaultWebtransportUnidStreamFrame(in.readRetainedSlice(in.readableBytes())));
      return;
    }
    if (type == -1) {
    ...
    switch (type) {
      ...
      case HTTP3_HEADERS_FRAME_TYPE:
        ...
        if (!enforceMaxPayloadLength(ctx, in, type, payLoadLength,
            // Let's use the maxHeaderListSize as a limit as this is this is the
            // decompressed amounts of
            // bytes which means the once we decompressed the headers we will be bigger then
            // the actual
            // payload size now.
      ...
      case HTTP3_SETTINGS_FRAME_TYPE:
        ...
        // Use 256 as this gives space for 16 maximal size encoder and 128 minimal size
        // encoded settings.
      ...
      case HTTP3_MAX_PUSH_ID_FRAME_TYPE:
        ...
        return payLoadLength;
      case WEBTRANSPORT_FRAME_TYPE_ID:
        WEBTRANSPORT_BID_STREAM = true;
        return payLoadLength;
      case WEBTRANSPORT_STREAM_TYPE:
        WEBTRANSPORT_UNID_STREAM = true;
        return payLoadLength;
      default:
   ...
   * @param in      {@link ByteBuf} containing the encode header block. It is
   *                assumed that the entire header block is
   *                contained in this buffer.
   * @param length  Number of bytes in the passed buffer that represent the
   *                encoded header block.
   * @param trailer {@code true} if this is a trailer section.
   * @return {@code true} if the headers were decoded, {@code false} otherwise. A
   *         header block may not be decoded if
  ...
  private void write0(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) {
    try {
      ...
      } else if (msg instanceof WebtransportBidStreamFrame) {
        writeBidStreamFrame(ctx, (WebtransportBidStreamFrame) msg, promise);
      } else if (msg instanceof WebtransportUnidStreamFrame) {
        writeUnidStreamFrame(ctx, (WebtransportUnidStreamFrame) msg, promise);
  ...
  private static <T extends Http3Frame> void writeDynamicFrame(ChannelHandlerContext ctx, long type, T frame,
    ...
    // Move 16 bytes forward as this is the maximum amount we could ever need for
    // the type + payload length.
  ...
  private static void writeUnidStreamFrame(ChannelHandlerContext ctx, WebtransportUnidStreamFrame frame,
      ChannelPromise promise) {
    QuicChannel quicChannel = (QuicChannel) ctx.channel().parent();
    webtransport.SendUnidH3(quicChannel, frame.content(), promise == null);
  }

  private static void writeBidStreamFrame(ChannelHandlerContext ctx, WebtransportBidStreamFrame frame,
      ChannelPromise promise) {
    ctx.writeAndFlush(frame.content(), promise);
  }
  ...
  private static final class WriteResumptionListener
    ...
    void drain() {
      ...
      try {
        ...
        // indicate that writes do not need to be enqueued. As we are on the eventloop,
        // no other writes can
