...
/**
 * {@link ByteToMessageDecoder} which helps to detect the type of unidirectional
 * stream.
 */
abstract class Http3UnidirectionalStreamInboundHandler extends ByteToMessageDecoder {
  ...
  private static final AttributeKey<Boolean> REMOTE_QPACK_DECODER_STREAM = AttributeKey
      .valueOf("H3_REMOTE_QPACK_DECODER_STREAM");
  private static final AttributeKey<Boolean> REMOTE_QPACK_ENCODER_STREAM = AttributeKey
      .valueOf("H3_REMOTE_QPACK_ENCODER_STREAM");

  private boolean WebTransportStream = false;
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
    ...
    if (WebTransportStream) {
      in.resetReaderIndex();
      initWebtransportStream(ctx);
      return;
    }
   ...
   * <a href=
   * "https://tools.ietf.org/html/draft-ietf-quic-http-32#section-6.2.1">control
   * stream</a>.
   */
  ...
  public void initWebtransportStream(ChannelHandlerContext ctx) {
    ctx.pipeline().replace(this, null,
        codecFactory.newCodec(Http3RequestStreamFrameTypeValidator.INSTANCE, NO_STATE, NO_STATE));
  };

  /**
   * Called if the current {@link Channel} is a
   * <a href=
   * "https://tools.ietf.org/html/draft-ietf-quic-http-32#section-6.2.2">push
   * stream</a>.
   */
   ...
   * Called if the current {@link Channel} is a
   * <a href=
   * "https://www.ietf.org/archive/id/draft-ietf-quic-qpack-19.html#name-encoder-and-decoder-streams">
