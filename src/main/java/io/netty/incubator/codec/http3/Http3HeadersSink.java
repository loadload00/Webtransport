...
final class Http3HeadersSink implements BiConsumer<CharSequence, CharSequence> {
    ...
    if (validate) {
      ...
      if (request) {
        ...
        if (HttpMethod.CONNECT.asciiName().contentEqualsIgnoreCase(method)) {
          ...
          // - :method
          // - :authority
          if ((pseudoHeadersCount != 2 || headers.authority() == null) && webtransport == false) {
            // There can't be any duplicates for pseudy header names.
            throw new Http3HeadersValidationException("Not all mandatory pseudo-headers included.");
          }
        } else {
          ...
          // - :authority
          // - :path
          if (pseudoHeadersCount != 4 && (pseudoHeadersCount == 5 && webtransport == false)) {
            // There can't be any duplicates for pseudy header names.
            throw new Http3HeadersValidationException("Not all mandatory pseudo-headers included.");
          }
        }
  ...
  public void accept(CharSequence name, CharSequence value) {
    ...
    if (name.toString().equals(":protocol") && value.toString().equals("webtransport")) {
      webtransport = true;
    }
  ...
  private void validate(Http3Headers headers, CharSequence name) {
    if (hasPseudoHeaderFormat(name)) {
      ...
      final HeaderType currentHeaderType = pseudoHeader.isRequestOnly() ? HeaderType.REQUEST_PSEUDO_HEADER
          : HeaderType.RESPONSE_PSEUDO_HEADER;
      if (previousType != null && currentHeaderType != previousType && !webtransport) {
